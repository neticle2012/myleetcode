...
## 复杂度分析
- 时间复杂度：$O(N)$，其中$N$为数组元素个数，线性遍历`nums`使用$O(N)$的时间，求出`x`和`y`的二进制位使用$O(32)=O(1)$的时间，因为`x`和`y`都是`int`型，长度都为`4`字节`32`位，最坏的情况是`x`和`y`其他位相同只有最高位不同（即$x=0\underbrace{0 \cdots 0}_\text{31个}, y=1\underbrace{0 \cdots 0}_\text{31个}$），则`ref`需要算数左移`32`次
- 空间复杂度：$O(1)$