...
## 基本思路
维护一个存储`Node`对象（每个`Node`对象有`heights`数组的元素`val`和<strong>以该元素为高度可以勾勒出的最大矩形的起始下标`prevTime`</strong>两个属性）的单调栈`stack`，<strong>从栈底到栈顶的下标对应的的温度依次递增</strong>。如果一个`Node`对象在单调栈里，则表示尚未找到该`height`数组元素可以勾勒出的最大矩形。整个算法的核心是，<strong>每次有新元素进栈时，会将`val`更大的元素全部出栈，并计算出出栈元素的面积去更新面积最大值</strong>
