...
## 基本思路
在我的思路上进一步做优化：可以让单调栈只保存`height`数组的元素的下标`i`，<strong>从栈底到栈顶的`height[i]` 依次递减</strong>，遍历`height`数组，对于每个元素`height[i]`，
...
- 如果`height[i]` $>$ 当前栈顶的值`height[stack.peek()]`，则说明对于该栈顶元素`height[stack.peek()]`而言，其上方的水量的高度上限至少在`height[i]`和将该栈顶元素出栈后新的栈顶元素`height[stack.新peek()]`的最小值以上，但是具体上限在哪里我们还不知道，所以这一次先计算高度区间$[height[stack.peek()], Math.min(height[stack.新peek()], height[i])]$的水量
	1. 先将栈顶元素`pop`出栈，然后按照上述说明计算水量，但是<strong>注意水量的宽度区间为$stack.新peek() + 1, stack.peek()$</strong>，因此`Area = (Math.min(height[stack.新peek()], height[i]) - height[stack.peek()]) * (stack.peek() - stack.新peek())`，将该水量累积到总水量中
